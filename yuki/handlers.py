...
def _clean_llm_reply(text: str) -> str:
    ...
    # Anything after the first divider line is meta-narration — drop it.
    text = _DIVIDER_RE.split(text, maxsplit=1)[0]
    # Drop lines that are ONLY *stage direction*
    kept = [ln for ln in text.split("\n") if not _STAGE_LINE_RE.match(ln)]
    return "\n".join(kept).strip()


def _split_into_bursts(text: str, max_bursts: int = 4) -> list[str]:
...
def handle_free_text(chat_id: int, user_id: int, text: str) -> None:
    ...
    bursts = _split_into_bursts(cleaned)
